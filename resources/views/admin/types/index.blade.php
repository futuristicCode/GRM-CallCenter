...
@section('title', __('Types de réclamation'))

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('Types de réclamation') }}</h1>
        <p class="text-sm text-gray-500 mt-1">{{ __('Gérez les types et sous-types de réclamations') }}</p>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-900">{{ __('Ajouter un type') }}</h3>
                    </div>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.types.store') }}">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="text" name="code" required placeholder="{{ __('Ex: BILL') }}" class="input w-full">
                            </div>
                            <div>
                                <label class="label">{{ __('Libellé') }}</label>
                                <input type="text" name="libelle" required placeholder="{{ __('Ex: Billeterie') }}" class="input w-full">
                            </div>
                            <div>
                                <label class="label">{{ __('SLA (heures)') }}</label>
                                <input type="number" name="delai_traitement_sla" value="72" min="1" required class="input w-full">
                            </div>
                            <div class="flex items-end">
                                <button type="submit" class="btn-primary w-full">{{ __('Créer') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-900">{{ __('Types existants') }}</h3>
                        <span class="badge-blue {{ app()->getLocale() === 'ar' ? 'mr-auto' : 'ml-auto' }}">{{ $types->count() }} {{ __('type(s)') }}</span>
                    </div>
                </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('SLA (h)') }}</th>
                                <th>{{ __('Statut') }}</th>
                                <th class="{{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }}">{{ __('Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($types as $type)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="{{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }}">
                                        <div class="flex items-center gap-2 {{ app()->getLocale() === 'ar' ? 'justify-start' : 'justify-end' }}">
                                            <form method="POST" action="{{ route('admin.types.update', $type) }}">
                                                ...
                                                <button type="submit" class="btn-sm {{ $type->actif ? 'bg-amber-50 text-amber-700 hover:bg-amber-100' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100' }} border-0">
                                                    {{ $type->actif ? __('Désactiver') : __('Activer') }}
                                                </button>
                                            </form>
                                            <form method="POST" action="{{ route('admin.types.destroy', $type) }}" onsubmit="return confirm('{{ __('Supprimer ce type ?') }}')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn-sm btn-danger">{{ __('Supprimer') }}</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center text-gray-400 py-8">{{ __('Aucun type trouvé.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-semibold text-gray-900">{{ __('Sous-types') }}</h3>
                    </div>
                </div>
                    ...
                    @forelse($types as $type)
                        <div class="border border-gray-200 rounded-xl p-4">
                            ...
                            <form method="POST" action="{{ route('admin.types.sous-types.store') }}" class="flex gap-2 mb-3">
                                ...
                                <input type="text" name="libelle" required placeholder="{{ __('Nouveau sous-type...') }}" class="input flex-1 text-sm">
                                ...
                                <div class="space-y-1">
                                    @foreach($type->sousTypes as $st)
                                        <div class="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-gray-50">
                                            ...
                                            @if($st->actif)
                                                <form method="POST" action="{{ route('admin.types.sous-types.destroy', $st) }}" onsubmit="return confirm('{{ __("Supprimer ?") }}')">
                                                    @csrf
                                                    @method('DELETE')
                                            ...
                                            @else
                                                <span class="badge-gray text-[10px]">{{ __('Inactif') }}</span>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-xs text-gray-400 italic">{{ __('Aucun sous-type') }}</p>
                            @endif
                        </div>
                    @empty
                        <div class="text-center py-6 text-gray-400 text-sm">{{ __('Aucun type trouvé.') }}</div>
                    @endforelse
                </div>
            </div>
        </div>
