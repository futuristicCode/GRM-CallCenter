            ...
            <form method="GET" action="{{ route('admin.users.index') }}">
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nom ou email..." class="input w-full">
                    </div>
                    <div>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td><span class="{{ $roleColors[$user->role] ?? 'badge-gray' }}">{{ ucfirst($user->role) }}</span></td>
                            <td>
                                @if($user->is_active)
                            ...
                            <td class="text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    {{-- Modifier --}}
                                    <a href="{{ route('admin.users.edit', $user) }}" class="btn-sm bg-indigo-50 text-indigo-600 hover:bg-indigo-100 border-0" title="Modifier">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"/></svg>
                                    </a>

                                    {{-- Réinit MDP --}}
                                    <form action="{{ route('admin.users.reset-password', $user) }}" method="POST" onsubmit="return confirm('Réinitialiser le mot de passe à \"password\" ?')">
                                        @csrf
                                        <button type="submit" class="btn-sm bg-amber-50 text-amber-600 hover:bg-amber-100 border-0" title="Réinitialiser MDP">
