        ...
        <div class="card-header">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400">{{ ucfirst($user->role) }} · {{ $user->is_active ? 'Actif' : 'Inactif' }}</p>
                </div>
            </div>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.users.update', $user) }}">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="input" placeholder="+243 ...">
                        @error('phone')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-100">
                        <label for="is_active" class="inline-flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="badge {{ $user->is_active ? 'badge-green' : 'badge-red' }}">{{ $user->is_active ? 'Actif' : 'Inactif' }}</span>
                        </label>
                        <p class="text-xs text-gray-400 mt-1.5 ml-14">Désactivez pour empêcher la connexion de cet utilisateur</p>
                    </div>
                </div>
